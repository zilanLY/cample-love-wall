...
<body>
    ...
    <div id="adminPage" class="admin-layout" style="display:none;">
        <div class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a data-page="comments" onclick="switchPage('comments')">
                    <span class="icon">💬</span><span>评论管理</span>
                </a></li>
        ...
        <div class="main-content">
            ...
            <div id="page-comments" class="page" style="display:none;">
                ...
                <div class="card">
                    <div class="filter-bar">
                        <select id="commentsStatusFilter" onchange="loadComments()">
                            <option value="">全部状态</option>
                            <option value="pending">待审核</option>
                            <option value="approved">已通过</option>
                            <option value="rejected">已驳回</option>
                        </select>
                        <input type="text" class="search-input" id="commentsSearch" placeholder="搜索评论..." oninput="debounceLoadComments()">
                    </div>
                    <div class="table-container">
                        <table>
                            <thead>
            ...
            <div id="page-settings" class="page" style="display:none;">
                ...
                <div class="card">
                    <div class="card-header"><h3>内容设置</h3></div>
                    <div class="config-item">
                        <div>
                    ...
                    <div class="config-item">
                        <div>
                            <div class="config-label">启用人工审核</div>
                            <div class="config-desc">开启后内容需要人工审核才能展示</div>
                        </div>
                        <label class="switch">
                            <input type="checkbox" id="manualReviewEnabled">
                            <span class="slider"></span>
                        </label>
                    </div>
                </div>
                <div class="card">
                    ...
                    <div class="config-item">
                        <div>
                            <div class="config-label">启用评论功能</div>
                            <div class="config-desc">开启后用户可以发表评论</div>
                        </div>
                        <label class="switch">
                            <input type="checkbox" id="commentEnabled" checked>
                            <span class="slider"></span>
                        </label>
                    </div>
                    <div class="config-item">
                        <div>
                            <div class="config-label">评论需要审核</div>
                            <div class="config-desc">开启后评论需要审核才能展示</div>
                        </div>
                        <label class="switch">
                            <input type="checkbox" id="commentNeedReview" checked>
                            <span class="slider"></span>
                        </label>
                    </div>
                    <div class="config-item">
                        <div>
                            <div class="config-label">评论最大长度</div>
                            <div class="config-desc">用户发表评论的最大字数限制</div>
                        </div>
                        <input type="number" id="commentMaxLength" value="200" style="width:100px;padding:8px;border-radius:8px;border:1px solid #ddd;">
                    </div>
                    <div class="config-item">
                        <div>
                            <div class="config-label">评论频率限制</div>
                            <div class="config-desc">每小时最多发表评论数量</div>
                        </div>
                        <input type="number" id="commentRateLimit" value="10" style="width:100px;padding:8px;border-radius:8px;border:1px solid #ddd;">
                    </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h3>图片存储设置</h3>
                        <button class="btn btn-primary btn-sm" onclick="openGitHubRepoModal()">+ 添加仓库</button>
                    </div>
                    
                    <div class="config-item">
                        <div>
                            <div class="config-label">仓库选择策略</div>
                            <div class="config-desc">多个仓库时如何选择使用哪个仓库</div>
                        </div>
                        <select id="repoStrategy" style="padding:8px;border-radius:8px;border:1px solid #ddd;">
                    ...
                    <div style="margin-top:20px;">
                        <div class="config-label" style="margin-bottom:10px;">仓库列表</div>
                        <div class="table-container">
                            <table>
                                <thead>
                                    <tr>
                                ...
                                <tbody id="githubReposTableBody">
                                    <tr>
                                        <td colspan="6" style="text-align:center;color:#999;padding:30px;">
                                            暂无仓库配置，点击上方「添加仓库」按钮添加
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    <div style="margin-top:20px;text-align:right;">
                        <button class="btn btn-primary" onclick="saveGitHubRepos()">保存存储设置</button>
                    </div>
                </div>
    ...
    <div class="modal-overlay" id="githubRepoModal">
        <div class="modal" style="max-width:500px;">
            <div class="modal-header">
                <h3 id="githubRepoModalTitle">添加仓库</h3>
                <button class="modal-close" onclick="closeModal('githubRepoModal')">&times;</button>
            </div>
            <div class="modal-body">
                <input type="hidden" id="githubRepoId">
                <div class="form-group">
                    <label>仓库名称</label>
                    <input type="text" id="githubRepoName" placeholder="例如：主图仓库">
                </div>
                <div class="form-group">
                    <label>GitHub Token</label>
                    <input type="password" id="githubRepoToken" placeholder="ghp_xxxxxxxxxxxx">
                </div>
                <div class="form-group">
                    <label>仓库所有者</label>
                    <input type="text" id="githubRepoOwner" placeholder="username">
                </div>
                <div class="form-group">
                    <label>仓库名称</label>
                    <input type="text" id="githubRepoRepo" placeholder="images-repo">
                </div>
                <div class="form-group" style="display:grid;grid-template-columns:1fr 1fr;gap:15px;">
                    <div>
                        <label>分支名称</label>
                        <input type="text" id="githubRepoBranch" value="main">
                    </div>
                    <div>
                        <label>权重</label>
                        <input type="number" id="githubRepoWeight" value="1" min="1">
                    </div>
                </div>
                <div class="form-group">
                    <label>存储路径前缀</label>
                    <input type="text" id="githubRepoPath" value="images">
                </div>
                <div class="config-item">
                    <div>
                        <div class="config-label">使用 jsDelivr CDN</div>
                        <div class="config-desc">启用后通过 jsDelivr 加速访问</div>
                    </div>
                    <label class="switch">
                        <input type="checkbox" id="githubRepoUseJsDelivr" checked>
                        <span class="slider"></span>
                    </label>
                </div>
                <div class="config-item">
                    <div>
                        <div class="config-label">启用仓库</div>
                        <div class="config-desc">禁用后该仓库不会被使用</div>
                    </div>
                    <label class="switch">
                        <input type="checkbox" id="githubRepoEnabled" checked>
                        <span class="slider"></span>
                    </label>
                </div>
                <div style="margin-top:15px;">
                    <button class="btn btn-secondary btn-sm" onclick="testGitHubRepoConnection()">测试连接</button>
                    <span id="testResult" style="margin-left:10px;font-size:13px;"></span>
                </div>
            </div>
            <div class="modal-footer">
                <button class="btn btn-secondary" onclick="closeModal('githubRepoModal')">取消</button>
                <button class="btn btn-primary" onclick="saveGitHubRepo()">保存</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        window.addEventListener('hashchange', function() {
            if (!token) return;
            const hash = location.hash.replace('#/', '');
            if (hash && document.getElementById('page-' + hash) && hash !== currentPage) {
                switchPage(hash);
            }
        });
        ...
        async function loadComments() {
            const status = document.getElementById('commentsStatusFilter').value;
            const search = document.getElementById('commentsSearch').value.trim();
            const params = new URLSearchParams({ page: currentCommentsPage, pageSize: 10 });
            if (status) params.append('status', status);
            if (search) params.append('search', search);
            const data = await apiRequest('/api/admin/comments?' + params.toString());
            if (!data || data.code !== 0) return;
            const tbody = document.getElementById('commentsTableBody');
            const comments = data.data.list;
            if (comments.length === 0) {
                tbody.innerHTML = '<tr><td colspan="8"><div class="empty-state"><div class="emoji">💬</div><p>暂无评论</p></div></td></tr>';
            } else {
                tbody.innerHTML = comments.map(comment => '<tr><td>' + comment.id + '</td><td style="max-width:250px;"><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escapeHtml(comment.content) + '</div></td><td>' + (comment.post_title || '-') + '</td><td>' + escapeHtml(comment.author || '匿名') + '</td><td><span class="status-badge ' + comment.status + '">' + (comment.status === 'pending' ? '待审核' : comment.status === 'approved' ? '已通过' : '已驳回') + '</span></td><td>' + comment.likes + '</td><td>' + formatDate(comment.created_at) + '</td><td><div class="action-btns"><button class="btn btn-success btn-sm" onclick="approveComment(' + comment.id + ')">通过</button><button class="btn btn-danger btn-sm" onclick="rejectComment(' + comment.id + ')">驳回</button><button class="btn btn-secondary btn-sm" onclick="deleteComment(' + comment.id + ')">删除</button></div></td></tr>').join('');
            }
            renderPagination('commentsPagination', data.data, currentCommentsPage, (page) => { currentCommentsPage = page; loadComments(); });
        }

        let commentsSearchTimer;
        function debounceLoadComments() {
            clearTimeout(commentsSearchTimer);
            commentsSearchTimer = setTimeout(() => { currentCommentsPage = 1; loadComments(); }, 500);
        }

        async function approveComment(id) {
            const data = await apiRequest('/api/admin/comments/' + id + '/review', {
                method: 'PUT',
        ...
        async function deleteComment(id) {
            ...
            const data = await apiRequest('/api/admin/comments/' + id, { method: 'DELETE' });
            if (data && data.code === 0) {
                showToast('删除成功', 'success');
                loadComments();
            }
        ...
        async function loadSettings() {
            ...
            document.getElementById('manualReviewEnabled').checked = settings.manual_review_enabled !== false;
            document.getElementById('commentEnabled').checked = settings.comment_enabled !== false;
        ...
        async function saveSettings() {
            const settings = {
                ...
                manual_review_enabled: document.getElementById('manualReviewEnabled').checked,
                comment_enabled: document.getElementById('commentEnabled').checked,
                comment_need_review: document.getElementById('commentNeedReview').checked,
                comment_max_length: parseInt(document.getElementById('commentMaxLength').value) || 200,
                comment_rate_limit: parseInt(document.getElementById('commentRateLimit').value) || 10
            };
        ...
        async function loadGitHubRepos() {
            try {
                const data = await apiRequest('/api/admin/storage/github/repos');
                if (data && data.code === 0) {
                    githubRepos = data.data.repos || [];
                    document.getElementById('repoStrategy').value = data.data.strategy || 'round_robin';
                    renderGitHubReposTable();
                }
            } catch (e) {
                console.error('加载 GitHub 仓库配置失败:', e);
            }
        }

        function renderGitHubReposTable() {
            const tbody = document.getElementById('githubReposTableBody');
            ...
            tbody.innerHTML = githubRepos.map(repo => \`
                <tr>
                    <td>\${repo.name || '未命名'}</td>
                    ...
                    <td>\${repo.weight || 1}</td>
                    <td>
                        <span class="status-badge \${repo.enabled !== false ? 'approved' : 'rejected'}">
                            \${repo.enabled !== false ? '启用' : '禁用'}
                        </span>
                    </td>
                    <td>
                        <div class="action-btns">
                            <button class="btn btn-sm btn-secondary" onclick="testGitHubRepo('\${repo.id}')">测试</button>
                            <button class="btn btn-sm btn-secondary" onclick="openGitHubRepoModal('\${repo.id}')">编辑</button>
                            <button class="btn btn-sm btn-danger" onclick="deleteGitHubRepo('\${repo.id}')">删除</button>
                        </div>
                    </td>
                </tr>
            \`).join('');
        ...
        function openGitHubRepoModal(repoId) {
            const modal = document.getElementById('githubRepoModal');
            const title = document.getElementById('githubRepoModalTitle');
            ...
            if (repoId) {
                ...
                document.getElementById('githubRepoId').value = repo.id;
                document.getElementById('githubRepoName').value = repo.name || '';
                document.getElementById('githubRepoToken').value = repo.token || '';
                document.getElementById('githubRepoOwner').value = repo.owner || '';
                document.getElementById('githubRepoRepo').value = repo.repo || '';
                document.getElementById('githubRepoBranch').value = repo.branch || 'main';
                document.getElementById('githubRepoPath').value = repo.path || 'images';
                document.getElementById('githubRepoWeight').value = repo.weight || 1;
                document.getElementById('githubRepoUseJsDelivr').checked = repo.useJsDelivr !== false;
                document.getElementById('githubRepoEnabled').checked = repo.enabled !== false;
            } else {
                // 添加模式
                title.textContent = '添加仓库';
                document.getElementById('githubRepoId').value = '';
                document.getElementById('githubRepoName').value = '';
                document.getElementById('githubRepoToken').value = '';
                document.getElementById('githubRepoOwner').value = '';
                document.getElementById('githubRepoRepo').value = '';
                document.getElementById('githubRepoBranch').value = 'main';
                document.getElementById('githubRepoPath').value = 'images';
                document.getElementById('githubRepoWeight').value = 1;
                document.getElementById('githubRepoUseJsDelivr').checked = true;
                document.getElementById('githubRepoEnabled').checked = true;
            }
        ...
        function saveGitHubRepo() {
            const id = document.getElementById('githubRepoId').value;
            const repoData = {
                name: document.getElementById('githubRepoName').value.trim(),
                token: document.getElementById('githubRepoToken').value.trim(),
                owner: document.getElementById('githubRepoOwner').value.trim(),
                repo: document.getElementById('githubRepoRepo').value.trim(),
                branch: document.getElementById('githubRepoBranch').value.trim() || 'main',
                path: document.getElementById('githubRepoPath').value.trim() || 'images',
                weight: parseInt(document.getElementById('githubRepoWeight').value) || 1,
                useJsDelivr: document.getElementById('githubRepoUseJsDelivr').checked,
                enabled: document.getElementById('githubRepoEnabled').checked
            };
            ...
            if (id) {
                ...
                if (index !== -1) {
                    githubRepos[index] = { ...githubRepos[index], ...repoData };
                }
            } else {
                // 添加新仓库
                const newRepo = {
            ...
            renderGitHubReposTable();
            closeModal('githubRepoModal');
            showToast('仓库保存成功，记得点击「保存存储设置」按钮保存', 'success');
        }

        function deleteGitHubRepo(repoId) {
            if (!confirm('确定要删除这个仓库吗？')) return;
            
            githubRepos = githubRepos.filter(r => r.id !== repoId);
            renderGitHubReposTable();
            showToast('仓库已删除，记得点击「保存存储设置」按钮保存', 'success');
        }

        async function testGitHubRepo(repoId) {
            const repo = githubRepos.find(r => r.id === repoId);
            if (!repo) return;
            
            try {
                showToast('正在测试连接...', 'success');
                const data = await apiRequest('/api/admin/storage/github/test', {
                    method: 'POST',
                ...
                if (data && data.code === 0 && data.data.success) {
                    ...
                    showToast('测试失败：' + (data?.data?.message || '未知错误'), 'error');
                }
            } catch (e) {
        ...
        async function testGitHubRepoConnection() {
            ...
            if (!repoData.token || !repoData.owner || !repoData.repo) {
                showToast('请先填写 Token、所有者和仓库名称', 'error');
                return;
            }
            ...
            try {
                ...
                if (data && data.code === 0 && data.data.success) {
                    ...
                    resultEl.textContent = '✗ 测试失败：' + (data?.data?.message || '未知错误');
        ...
        async function saveGitHubRepos() {
            try {
                const strategy = document.getElementById('repoStrategy').value;
                const data = await apiRequest('/api/admin/storage/github/repos', {
                    method: 'PUT',
                    body: JSON.stringify({
                        repos: githubRepos,
