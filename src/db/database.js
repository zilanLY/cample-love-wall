...
export async function initDatabase(db) {
  ...
  for (const cat of categories) {
    await insertCategory.bind(...cat).run();
  }
  
  // 插入初始敏感词
  await db.prepare(`
    INSERT OR IGNORE INTO sensitive_words (word, category, level) VALUES (?, ?, ?)
  `).bind('违禁词1', 'other', 2).run();
  
  await db.prepare(`
    INSERT OR IGNORE INTO sensitive_words (word, category, level) VALUES (?, ?, ?)
  `).bind('违禁词2', 'other', 2).run();
  ...
  for (const [key, value] of settings) {
    await insertSetting.bind(key, value).run();
  }
  
  return { success: true, message: '数据库初始化成功', alreadyInitialized: false };
}
